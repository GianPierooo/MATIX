...
# ── Gate: las consecuentes NO se ejecutan sin confirmado=true ────────────────


def test_mover_sin_confirmar_bloquea(area):
    ...
    # No se movió nada: el gate bloqueó ANTES de tocar el disco.
    assert (area / "f.txt").exists()
    assert not (area / "g.txt").exists()


def test_consecuentes_todas_exigen_gate(area):
    ctx = Contexto(allowlist=[area])
    for nombre, args in [
        ("mover_archivo", {"origen": str(area / "a"), "destino": str(area / "b")}),
        ("renombrar_archivo", {"ruta": str(area / "a"), "nuevo_nombre": "b"}),
        ("crear_carpeta", {"ruta": str(area / "nueva")}),
        ("organizar_aplicar", {"carpeta": str(area), "criterio": "por tipo"}),
    ]:
        res = _run(nombre, args, ctx)  # sin confirmado
        assert res["tipo"] == "requiere_confirmacion", nombre
